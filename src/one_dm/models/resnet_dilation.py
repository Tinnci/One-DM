...
class ResnetDilated(nn.Module):
    ...
    def __init__(self, orig_resnet, dilate_scale=8):
        ...
        # 除了最后一层外，将前面层的膨胀率按照设置扩大
        if dilate_scale == 8:
            self.orig_resnet.layer3.apply(self._dilate_func(2))
            self.orig_resnet.layer4.apply(self._dilate_func(4))
        elif dilate_scale == 16:
            self.orig_resnet.layer4.apply(self._dilate_func(2))
    
    def _dilate_func(self, dilation):
        def _set_dilate(m):
