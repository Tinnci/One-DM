...
def main():
    # 输出目录
    output_dir = "outputs/custom_paragraphs"
    os.makedirs(output_dir, exist_ok=True)
    
    # 读取自定义段落内容
    custom_paragraph_path = "data/english_paragraph/content/custom_paragraph.txt"
    with open(custom_paragraph_path, 'r', encoding='utf-8') as f:
        custom_paragraph = f.read().strip().split('\n')
    ...
    if os.name == 'nt':  # Windows
        ...
        for font_name in ['arial.ttf', 'times.ttf', 'comic.ttf', 'calibri.ttf', 'verdana.ttf']:
    ...
    # 为每种字体生成段落样本
    for font_idx, font_path in enumerate(system_font_paths):  # 使用所有找到的字体
        font_name = os.path.basename(font_path).split('.')[0] if font_path != "default" else "default"
        font_size = 30  # 使用固定大小，便于比较
        
        for align_idx, alignment_name in enumerate(alignment_names):
            output_path = os.path.join(output_dir, f"{font_name}_custom_{alignment_name}.png")
    ...
    print("自定义样本段落生成完成，请查看输出目录：", output_dir)

if __name__ == "__main__":
    try:
        main()
        # 将完成标记写入文件
        with open("custom_generation_complete.txt", "w") as f:
            f.write("自定义段落生成完成！")
    except Exception as e:
        print(f"程序出现错误: {e}")
        import traceback
        traceback.print_exc()
        # 将错误写入文件
        with open("custom_generation_error.txt", "w") as f:
            f.write(f"程序运行出错: {e}\n")
            import traceback
